...
const getNames = (arr) => {
    ...
    const newArr = arr.map(Element => {
        return Element.name.split('').reduce((rev, val) => val + rev, "");
    })
    return newArr;
...
const count = (target, input) => {
    // Solution code here...
    let count = 0;
    let arr1 = input.map(Element => {
        let arr2 = Element.filter((val, idx) => {
            if (val === target)
                count++;
            return true;
        })
...
const totalSum = (input) => {
    ...
    let countNum = 0;
    let arr1 = input.map(Element => {
        let summation = Element.reduce((acc, val) => {
            acc = acc + val;
            return acc;
        }, 0);
        countNum = countNum + summation;
        return countNum;
    })
    return countNum;
};
...
let findMaleAndFemale = (data) => {
    // Solution code here...
    let arr0 = data.filter(element => {
        if (element.gender == 'female' || element.gender == 'male') {
            return true;
        }
    });

    let result = arr0.reduce((acc, val, idx) => {
        acc[idx] = val.name;
        return acc;
    }, []);
...
let findShortest = (data) => {
    // Solution code here...
    let minimum = data[0].name;
    let arr = data.map(element => {
        return element.name;
    })
    let arr2 = arr.filter(element => {
        if (minimum.length >= element.length) {
            minimum = element;
            return element;
        }
    })

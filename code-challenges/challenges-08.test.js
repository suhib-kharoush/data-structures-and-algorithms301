...
function returnTen(str) {
    // Solution code here...
    if (str.length >= 10) {
        let strings = str.split('').splice(1);
        return strings;
    } else {
...
const filterStringsWithVowels = (arr) => {
    // Solution code here...
    let removeVowels = arr.filter((Element) => {
        if (Element.includes("a") || Element.includes("e") || Element.includes("i") || Element.includes("u") || Element.includes("o")) {
            return true;
        }
    })
    return removeVowels;
...
const notInFirstArray = (forbiddenValues, arr) => {
    // Solution code here...
    let secondArr = arr.filter((Element) => {
